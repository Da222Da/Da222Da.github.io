<template><h1 id="file-文件对象" tabindex="-1"><a class="header-anchor" href="#file-文件对象" aria-hidden="true">#</a> File 文件对象</h1>
<div class="custom-container tip"><p class="custom-container-title">TIP</p>
<nav class="table-of-contents"><ul><li><RouterLink to="#overall">Overall</RouterLink><ul><li><RouterLink to="#_01-web-应用程序操作文件的方式">01.Web 应用程序操作文件的方式</RouterLink></li><li><RouterLink to="#_02-file-api">02.File API</RouterLink></li><li><RouterLink to="#_03-为什么会有-blob-数据类型">03.为什么会有 Blob 数据类型？</RouterLink></li></ul></li><li><RouterLink to="#use">Use</RouterLink><ul><li><RouterLink to="#_01-使用-input-元素选取文件时-那些文件信息会被绑定到-input-元素之上">01. 使用 input 元素选取文件时，那些文件信息会被绑定到 input 元素之上？</RouterLink></li><li><RouterLink to="#_02-上传一张名为-image-png-的图片时-input-value">02. 上传一张名为 image.png 的图片时，input.value = ?</RouterLink></li><li><RouterLink to="#_03-例子-如何从文件系统中读取图片数据-并在网页向用户展示">03. (例子)如何从文件系统中读取图片数据，并在网页向用户展示？</RouterLink></li><li><RouterLink to="#_04-如何声明一个-blob-对象">04.如何声明一个 Blob 对象？</RouterLink></li><li><RouterLink to="#_05-例子-截取纯文本内容的前-20-个字节-并打印">05.(例子) 截取纯文本内容的前 20 个字节, 并打印？</RouterLink></li></ul></li></ul></nav>
</div>
...
<ul>
<li>用户手动将文件提交到<code>&lt;input type='file' /&gt;</code>，然后借助 File API 读取文件信息。</li>
...
<ul>
<li>
<p>FileReader 数据类型<code>一种异步文件读取机制，从文件系统读取文件</code></p>
<ul>
<li>FileReader 对象上的异步方法：
<ul>
<li><code>readAsText(file, encoding)</code> 从文件中读取纯文本内容并保存在 result 属性中。</li>
...
<li><code>readAsArrayBuffer(file)</code>读取文件并将文件内容以 ArrayBuffer 形式保存在 result 属性。</li>
</ul>
</li>
<li>因为这些读取方法是异步的，所以每个 FileReader 会发布几个事件，最常见的有:
<ul>
<li><code>progress 事件</code>每 50 毫秒就会触发一次，其与 XHR 的 progress 事件具有相同的信息：</li>
<li><code>error 事件</code>会在由于某种原因无法读取文件时触发。</li>
<li><code>load 事件</code>会在文件成功加载后触发。</li>
</ul>
</li>
</ul>
</li>
<li>
<p>FileReaderSync 数据类型<code>与FileReader相似，只是读取文件的机制是同步</code></p>
</li>
</ul>
...
<p>input 元素 =&gt; <code>&lt;input type='file' /&gt;</code></p>
<p>当用户选择一个或多个文件时，<code>e.target.files = [] &amp;&amp; e.target.files.push(File对象)</code></p>
<p>每一个 File 对象都存了一些选中文件的信息，例如<code>文件名name、文件类型type、文件大小size</code></p>
...
<p>当我们上传一个名为 image.png 的图片时，打印时会发现<code>input.value='C:\fakepath\image.png'</code></p>
<p>这里的<code>fakepath</code>表示伪路径。</p>
